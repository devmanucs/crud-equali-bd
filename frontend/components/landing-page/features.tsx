...
export function Features() {
  return (
    <section className="container mx-auto px-4 py-16">
      <h2 className="font-fredoka text-2xl md:text-3xl font-bold text-center text-[#1f2937] mb-4">
...
// O sub-componente fica aqui dentro pois só é usado aqui
function FeatureCard({ icon, title, description, color }: { icon: React.ReactNode, title: string, description: string, color: string }) {
  return (
    <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:shadow-md transition-shadow">
      <div className={`${color} p-3 rounded-xl mb-4`}>
        {icon}
      </div>
      <h3 className="font-fredoka font-bold text-lg text-[#1f2937] mb-2">{title}</h3>
